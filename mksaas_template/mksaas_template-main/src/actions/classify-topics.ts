...
import { STATIC_CATEGORIES, suggestCategoryByKeywords } from '@/config/knowledge-categories';
...
const CATEGORY_GUIDE = STATIC_CATEGORIES.map((cat) => `- ${cat.name} (${cat.slug}): ${cat.description}`).join('\n');
...
export async function classifyTopics(topics: TopicInput[]): Promise<ClassifiedTopic[]> {
