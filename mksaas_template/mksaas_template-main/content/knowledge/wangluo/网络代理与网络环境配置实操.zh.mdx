...
<Callout type="info">
网络环境纯净度是决定账号“能否注册、能否保号”的关键。注册前先在 `scamalytics.com`、`ipjiance.com`、`ipinfo.io`、`ping0.cc`、`ip111.cn` 全面检测，风险值 0–25 为低风险，超过 40 建议直接切换网络环境。
</Callout>
- 确保客户端支持系统级代理（TUN）。Claude Code、Dia 浏览器等桌面应用会绕开浏览器，在未开启 TUN 时常表现为“网页能用、终端报错”。
- 网络策略要配合账号策略：新号尽量固定在 1–2 个网络环境上，切换网络环境前先退出所有敏感服务。

<Callout type="info" title="🔧 客户端与配置建议">
- **Mac**：Surge（稳定但需订阅）、Clash Pro、Clash Verge 均内置 TUN 模式；Clash X 不支持 TUN，建议升级。
- **Windows**：选择支持系统代理的客户端（Clash for Windows、NekoRay 等），并启用 TUN/系统代理，确保终端、WSL、VSCode 等都走网络代理工具。
- **TUN 配置提醒**：在 Clash Verge 中右键菜单即可开启；开启后若出现断网，优先尝试切换网络环境或重启客户端。
</Callout>

### 网络环境选择与维护
- 地区优先级：美国 > 日本/欧洲 > 其他地区。Claude Code、Dia 对香港网络环境敏感，注册阶段尽量不用。

<Callout type="info">
- 保持多线路备用：机场一般月费 20–50 元；重要网络环境准备 2–3 条备用，遇到风控及时切换。
</Callout>
- 住宅 IP 获取方式：
  - 闲鱼购买住宅 IP（约 180 元/条），成功率高但需要频繁换号。
  - 使用 OCloud 等提供住宅网络环境的服务，支持 Cursor、GPT、Claude。
  - 自建 VPS + 住宅 IP 组合，适合长期稳定使用。

<Callout type="info">
- 玄学提醒：检测通过并不代表一定可用；一旦登录失败或提示异常，第一时间更换网络环境或客户端，而不是强行重试。
</Callout>
...
<Callout type="info">
**网络环境被识别**：当 Claude、ChatGPT 提示安全风险时，优先切换网络环境或换客户端。频繁重试同一 IP 只会放大风险。
</Callout>
...
<Callout type="info">
- 仅支持 Apple Silicon（M1/M2/M3/M4）芯片的 Mac；Intel Mac 会直接提示“无效应用”。
</Callout>
- 系统需升级到较新版本的 macOS。
- 网络环境必须为美国原生 IP；服务器或数据中心 IP 会报“地区受限”。

### 安装与注册步骤
1. 在启用美国网络环境的前提下访问 `[www.diabrowser.com](https://www.diabrowser.com/`) 下载客户端。
2. 注册阶段可以使用邀请码（示例：`[diabrowser.com](https://diabrowser.com/invite/BMOABT`、`https://diabrowser.com/invite/60T0CT`），也可以直接注册。)

<Callout type="info">
3. 注意注册页的“确认密码”字段并非邀请码；两次密码需一致。
</Callout>

<Callout type="info">
4. 双击安装包完成安装。若提示“无效”，确认是否为 Intel 芯片或系统版本过低。
</Callout>
5. 首次登录时保持美国原生网络环境，必要时重启客户端或电脑清理缓存。

### 常见问题与解决
- **登录白屏 / 右侧面板空白**：可能被广告或隐私规则拦截。关闭 EasyPrivacy、AdGuard Tracking Protection 等规则即可恢复。

<Callout type="warning">
- **网络环境受限**：更换到纯净的美国住宅网络环境，或重新检测当前网络环境的风险值。
</Callout>

<Callout type="warning">
- **注册失败**：再次确认密码填写无误、网络环境纯净，并避免短时间内反复提交。
</Callout>
...
<Callout type="info" title="付费策略与课程建议">
...
<Callout type="info">
- 开启后注意网络环境地区：部分 TUN 网络环境默认走香港，Claude Code 会直接拒绝。
</Callout>
...
- **网络环境地区**：当前是否为美国/英国等允许地区？香港网络环境直接换掉。
...
- **兜底操作**：新开对话 → 重启 VSCode → 重启电脑 → 切换网络环境 → 在终端直接调用 `claude` 命令测试。

<Callout type="info" title="⚡ 网络环境推荐方案">
...
### 💡 实战案例
- **Dia 无法登录**（考拉）：切换多个国家网络环境无效，最终更换为纯净的美国网络环境并开启 TUN 后成功。
- **电脑频繁断网**（王道春）：使用网络代理工具 1 小时后网络断开，排查后发现是客户端冲突，重新安装并更换软件解决。

<Callout type="warning">
- **Cursor 显示 “Not Available”**：在 Cursor 设置中将 Network 协议改为 HTTP/1.1，开启客户端 TUN 模式，使用美国/英国节点并保证风险值 ≥28，问题即可解决。
</Callout>

<Callout type="info" title="日常维护建议">

</Callout>
1. 开发时始终保持代理开启，尤其是需要访问 API 或 CLI 的工具。

<Callout type="warning">
2. 对 Claude、ChatGPT 等敏感服务固定使用美国网络环境，避免频繁切换。
</Callout>
3. 定期检测网络环境纯净度，任何异常立即切换网络环境。
4. 准备至少两套备用网络环境或备用客户端，出现异常时迅速切换。
5. 自建或长期使用时，关注公网 IP 是否被封；必要时更换供应商或购买新的住宅 IP。

通过上述配置与排查步骤，基本可以覆盖大多数代理、网络、网络环境相关问题，为后续账号注册、支付以及开发工具使用打下稳定基础。
