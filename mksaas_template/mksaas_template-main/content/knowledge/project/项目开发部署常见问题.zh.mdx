---
title: 项目开发部署常见问题
description: 环境搭建、本地开发、API接入到部署运维的常见问题解答
category: 项目执行
tags: ["出海开发"]
importance: 3
---
...
### Next.js 创建流程
1. 检查版本：`node -v`、`npm -v`。
2. 若网络慢，先执行 `npm config set registry https://registry.npmmirror.com`。
3. 创建项目：
   ```bash
...
4. 进入目录并启动：
   ...
   npm run dev
   ```bash
5. 浏览器访问 `http://localhost:3000`，看到页面即成功。
...
### 常见配置陷阱
- **连接失败**：免费账户通常限制 2 个 Project，若达到上限需删除多余项目。
- **Key 混淆**：务必区分 `API Key` 和 `Anon Key`。`.env` 中 `DATABASE_URL` 和 `NEXT_PUBLIC_SUPABASE_ANON_KEY` 必须正确对应。
- **权限控制**：不要在后台手动穷举权限。应在需求文档定义逻辑（如“未登录 1 次，登录 2 次”），通过代码逻辑（`is_admin`）或 RLS 策略实现。

### Vercel 连接池配置 (Serverless 必读)
- **隐性杀手锏**：Vercel 是 serverless 架构，会启动大量实例并发连接数据库，极易打爆连接数。
- **解决方案**：
  - 务必配置连接池（Connection Pool），限制每个服务实例的连接数。
  - 缩短 idle 超时时间。
  - **驱动选择**：强烈建议使用 `pg` 驱动，避免 `postgres` 驱动在 Vercel/Cloudflare 上的挂起 Bug。

### ID 类型选择
- `better-auth` 默认使用 text，Postgres 推荐 uuid。
- **结论**：只要保证唯一性，两者性能差异不大。关键是**早期统一**，避免后期改 Schema。
- **订单 ID**：建议使用可读结构（如 `orders-userid-date-seq`）而非纯 uuid，便于排查。
...
### Static 单页秒级发布
- 纯静态页面可直接把 HTML 保存为 `.html` 文件，拖拽到 [tiiny.site](https://tiiny.site/) 等免费托管平台，一键上线（泽伟分享）。

### 常见部署问题
- **全网 502 / queued 卡住**：先确认是不是供应商宕机（2024/10/20 AWS 宕机导致 Vercel/V0/Replicate 全挂）。确认是平台问题后耐心等待。遇到一直排队卡住，先看 [Vercel Status](https://www.vercel-status.com/)，别盲目改配置。
...
### Codex Bypass 模式风险
- **高危警告**：Codex 的 `bypass` 模式（`--dangerously-bypass-approvals-and-sandbox`）会扫描本地硬盘，存在极大安全隐患。
- **解决方案**：**必须在容器环境中使用**。
  - 推荐将开发环境容器化（Docker），安装 bash, git, openssh, python3, make, g++ 等基础工具。
  - 即使不用 bypass，容器化也能保持本地环境整洁。
