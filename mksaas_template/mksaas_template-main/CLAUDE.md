...
- **Daily Reports**: MDX-based daily reports system (see ROUTES.md for details)
  - Location: `src/app/[locale]/(docs)/reports/[[...slug]]/page.tsx`
  - Content: `content/reports/*.mdx`
  - Uses fumadocs for content management
...
**CRITICAL: Always check ROUTES.md before modifying routes**

- Daily reports use MDX files from `content/reports/` directory
- Route conflicts will cause "same specificity" errors
- See `ROUTES.md` for detailed routing structure and conflict prevention
- When adding new routes, verify no conflicts with existing routes
- Delete unused route implementations completely (don't just rename)
