...
const WebsiteInfoSchema = z.object({
  ...
  github: z.string().describe("The GitHub link of the website if it is open source"),
  categories: z
    .array(z.string())
...
export const fetchWebsiteInfo = async (url: string) => {
  ...
  const websiteInfo = WebsiteInfoSchema.parse({
    ...
    github: fetchedData.object.github,
...
export const fetchWebsiteInfoWithAI = async (url: string) => {
  try {
    ...
    const result = await generateObject({
      ...
      3. The GitHub link of the website if it is open source, otherwise leave it blank
      4. A detailed introduction in markdown format (include key features and use cases)
      5. Select appropriate categories from the available categories list (return array of names)
      6. Select relevant tags from the available tags list (return array of names)
