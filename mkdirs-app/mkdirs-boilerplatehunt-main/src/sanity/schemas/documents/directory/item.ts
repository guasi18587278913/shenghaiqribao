...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      name: "opensource",
      title: "Mark as Open Source",
      type: "boolean",
      initialValue: false,
      description:
        "If the item is open source, it will be displayed in the open source section",
    }),
    defineField({
      name: "github",
      title: "GitHub",
      type: "string",
      description: "The GitHub link of the item",
    }),
    defineField({
      name: "link",
      title: "Link",
    ...
    defineField({
      name: "basicPlanStatus",
      title: "Basic Plan Status",
      description: "The status of the item when the item is in basic plan",
      type: "string",
      group: "status",
      initialValue: "submitting",
      options: {
      ...
      hidden: ({ parent }) => parent.pricePlan !== "basic",
    }),
    defineField({
      name: "proPlanStatus",
  ...
  preview: {
    select: {
      ...
      basicPlanStatus: "basicPlanStatus",
    ...
    prepare({
      ...
      const error = freePlanStatus === "rejected" || basicPlanStatus === "failed" || proPlanStatus === "failed" || sponsorPlanStatus === "failed";
      const title = date ? `✅ ${name}` : error ? `❌ ${name}` : `⏳ ${name}`;
      const feature = featured ? "⭐" : "";
      const status = pricePlan.toUpperCase() === PricePlans.FREE.toUpperCase() ? freePlanStatus : pricePlan.toUpperCase() === PricePlans.BASIC.toUpperCase() ? basicPlanStatus : pricePlan.toUpperCase() === PricePlans.PRO.toUpperCase() ? proPlanStatus : sponsorPlanStatus;
      const time = date
