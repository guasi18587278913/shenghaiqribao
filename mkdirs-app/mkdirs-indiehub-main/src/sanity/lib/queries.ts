 ...
 * Tool Queries
 */
const toolTagFields = /* groq */ `
  ...,
`;

const toolCategoryFields = /* groq */ `
  ...,
`;

export const toolCollectionFields = /* groq */ `
  ...,
  icon {
    ...,
    "blurDataURL": asset->metadata.lqip,
...
// also used in file data/item.ts and data/submission.ts
export const toolSimpleFields = /* groq */ `
  _id,
  _createdAt,
  name,
  slug,
...
// auto generate related items
// TODO: fix this query, both are not working for fetching related tools in subcategory
// && subcategory._id == ^.^.subcategory._id 
// && subcategory._ref == ^.^.subcategory._ref 
const toolFieldsWithRelated = /* groq */ `
  introduction,
  // from indie hackers
  content,
...
export const toolByIdQuery = defineQuery(`*[_type == "tool" && _id == $id][0] {
  ${toolSimpleFields}
}`);

export const toolInfoBySlugQuery = defineQuery(`*[_type == "tool" && slug.current == $slug][0] {
  ${toolSimpleFields}
}`);

export const toolFullInfoByIdQuery = defineQuery(`*[_type == "tool" && _id == $id][0] {
  ${toolFields}
}`);

export const toolFullInfoBySlugQuery = defineQuery(`*[_type == "tool" && slug.current == $slug 
&& forceHidden != true] [0] {
  ${toolFieldsWithRelated}
}`);
 ...
 * if you want to change this query, please update data/item.ts
 */
export const toolListQuery = defineQuery(`*[_type == "tool" && defined(slug.current) 
  && defined(publishDate)
  && forceHidden != true]
  | order(coalesce(featured, false) desc, publishDate desc) {
    ${toolSimpleFields}
...
// get sponsor items
export const sponsorToolListQuery = defineQuery(`*[_type == "tool" && defined(slug.current) 
  && defined(publishDate)
  && forceHidden != true
  && sponsor == true
  && sponsorStartDate <= now()
...
export const toolListOfFeaturedQuery = defineQuery(`*[_type == "tool" && defined(slug.current) 
  && defined(publishDate) 
  && forceHidden != true 
  && featured == true] 
  | order(coalesce(featured, false) desc, publishDate desc) [0...$count] {
...
// coalesce(featured, false) desc, 
export const toolListOfLatestQuery = defineQuery(`*[_type == "tool" && defined(slug.current) 
  && defined(publishDate) 
  && forceHidden != true] 
  | order(publishDate desc) [0...$count] {
    ${toolSimpleFields}
...
export const toolCollectionListQuery = defineQuery(`*[_type == "toolCollection" && defined(slug.current)] 
  | order(priority desc) {
    ${toolCollectionFields}
}`);

export const toolCollectionQuery = defineQuery(`*[_type == "toolCollection" && slug.current == $slug][0] {
  ${toolCollectionFields}
}`);

export const toolCategoryListQuery = defineQuery(`*[_type == "toolCategory" && defined(slug.current)] 
  | order(priority desc) {
    ${toolCategoryFields}
}`);

export const toolCategoryWithSubCategoryFields = /* groq */ `
  ...,
...
export const toolCategoryListWithSubCategoryQuery = groq`*[_type=="toolCategory"] | order(priority desc, _createdAt asc) {
  ${toolCategoryWithSubCategoryFields}
}`;

export const toolCategoryQuery = defineQuery(`*[_type == "toolCategory" && slug.current == $slug][0] {
  ${toolCategoryFields}
}`);

export const toolTagListQuery = defineQuery(`*[_type == "toolTag" && defined(slug.current)] 
  | order(slug.current asc) {
    ${toolTagFields}
}`);

export const toolTagQuery = defineQuery(`*[_type == "toolTag" && slug.current == $slug][0] {
  ${toolTagFields}
}`);
...
export const toolListQueryForSitemap = groq`*[_type == "tool" && defined(slug.current) && defined(publishDate) && forceHidden != true] | order(_createdAt asc) {
  _id,
  _updatedAt,
  "slug": slug.current,
}`;

export const toolCategoryListQueryForSitemap = groq`*[_type == "toolCategory" && defined(slug.current)] | order(_createdAt asc) {
  _id,  
  _updatedAt,
  "slug": slug.current,
...
export const toolTagListQueryForSitemap = groq`*[_type == "toolTag" && defined(slug.current)] | order(_createdAt asc) {
  _id,  
  _updatedAt,
  "slug": slug.current,
  "count": count(*[_type == "tool" && defined(publishDate) && forceHidden != true && references(^._id)])
...
export const toolCollectionListQueryForSitemap = groq`*[_type == "toolCollection" && defined(slug.current)] | order(_createdAt asc) {
  _id,  
  _updatedAt,
  "slug": slug.current,
  "count": count(*[_type == "tool" && defined(publishDate) && forceHidden != true && references(^._id)])
