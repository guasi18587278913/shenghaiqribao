import { COLLECTIONS_PER_PAGE, ITEMS_PER_PAGE, TOOLS_PER_PAGE } from "@/lib/constants";
import type {
...
import {
  ...
  toolCategoryListQueryForSitemap,
  toolCollectionListQueryForSitemap,
  toolListQueryForSitemap,
  toolTagListQueryForSitemap,
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [
    ...
    sanityFetch<ToolListQueryForSitemapResult>({
      query: toolListQueryForSitemap,
    }),
    sanityFetch<ToolCategoryListQueryForSitemapResult>({
      query: toolCategoryListQueryForSitemap,
    }),
    sanityFetch<ToolTagListQueryForSitemapResult>({
      query: toolTagListQueryForSitemap,
    }),
    sanityFetch<ToolCollectionListQueryForSitemapResult>({
      query: toolCollectionListQueryForSitemap,
    }),
  ...
  console.log(`sitemap, app count:${itemListQueryResult.length}, pageCount:${pageCount}`);
  for (let i = 2; i <= pageCount; i++) {
    const routeUrl = `/app?page=${i}`;
    sitemapList.push({
      url: `${site_url}${routeUrl}`,
  ...
  for (const tool of toolListQueryResult) {
    if (tool.slug) {
      const routeUrl = `/tool/${tool.slug}`;
      // console.log(`sitemap, url:${site_url}${routeUrl}`);
      sitemapList.push({
        url: `${site_url}${routeUrl}`,
        lastModified: new Date(tool._updatedAt).toISOString(),
      });
    } else {
      console.warn(`sitemap, tool slug invalid, id:${tool._id}`);
    }
  }

  const toolPageCount = Math.ceil(toolListQueryResult.length / TOOLS_PER_PAGE);
  console.log(`sitemap, tool count:${toolListQueryResult.length}, pageCount:${toolPageCount}`);
  for (let i = 2; i <= toolPageCount; i++) {
    const routeUrl = `/tool?page=${i}`;
    sitemapList.push({
      url: `${site_url}${routeUrl}`,
      lastModified: new Date().toISOString(),
    });
  ...
  // tool collections
  for (const toolCollection of toolCollectionListQueryResult) {
    if (toolCollection.slug) {
      const routeUrl = `/collection/${toolCollection.slug}`;
      // console.log(`sitemap, url:${site_url}${routeUrl}`);
      sitemapList.push({
        url: `${site_url}${routeUrl}`,
        lastModified: new Date(toolCollection._updatedAt).toISOString(),
      });

      const pageCount = Math.ceil(toolCollection.count / COLLECTIONS_PER_PAGE);
      console.log(`sitemap, tool collection:${toolCollection.slug}, count:${toolCollection.count}, pageCount:${pageCount}`);
      for (let i = 2; i <= pageCount; i++) {
        const routeUrl = `/collection/${toolCollection.slug}?page=${i}`;
        sitemapList.push({
          url: `${site_url}${routeUrl}`,
          lastModified: new Date(toolCollection._updatedAt).toISOString(),
        });
      ...
      console.warn(`sitemap, tool collection slug invalid, id:${toolCollection._id}`);
    }
  }
