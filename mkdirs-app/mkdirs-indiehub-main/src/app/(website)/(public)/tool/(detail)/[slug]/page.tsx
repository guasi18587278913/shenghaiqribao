...
import ItemBreadCrumb from "@/components/tool/item-bread-crumb";
import SponsorItemCard from "@/components/tool/item-card-sponsor";
...
import {
  ...
  sponsorToolListQuery,
  toolFullInfoBySlugQuery,
  toolInfoBySlugQuery
} from "@/sanity/lib/queries";
import type { ToolFullInfo } from "@/types";
...
export async function generateMetadata({
  ...
  const item = await sanityFetch<ToolInfoBySlugQueryResult>({
    query: toolInfoBySlugQuery,
  ...
  return constructMetadata({
    ...
    canonicalUrl: `${siteConfig.url}/tool/${params.slug}`,
...
export default async function ItemPage({ params }: ItemPageProps) {
  ...
  // console.log("ItemPage, query:", toolFullInfoBySlugQuery);

  // if you support sponsor item, you can use this code
  const [item, sponsorItems] = await Promise.all([
    sanityFetch<ToolFullInfo>({
      query: toolFullInfoBySlugQuery,
      params: { slug: params.slug },
    }),
    // sanityFetch<SponsorToolListQueryResult>({
  ...
  const sponsorItem = sponsorItems?.length
    ...
    : null;
  const markdownContent = portableTextToMarkdown(item.content);
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3 flex flex-col">
          ...
          <div className="bg-muted/50 rounded-lg p-6 mr-0 lg:mr-8">
            ...
            {/* markdownContent, old tools */}
            {markdownContent && <ItemCustomMdx source={markdownContent} />}
            {/* introduction, new tools */}
            {item.introduction && <ItemCustomMdx source={item.introduction} />}
          </div>
              ...
              <div className="bg-muted/50 rounded-lg p-6">
                ...
                {item.subcategory && (
                  <Link
                    ...
                    className="text-sm link-underline"
                  >
                    {item?.categories?.[0]?.name} / {item.subcategory.name}
                  </Link>
                )}
              </div>
              ...
              <div className="bg-muted/50 rounded-lg p-6">
                ...
                <ul className="flex flex-wrap gap-4">
                  {item.tags?.map((tag) => (
                    <li key={tag._id}>
                      <Link
                        href={`/tool?tag=${tag.slug.current}`}
                        className="text-sm link-underline
      ...
      {item.related && item.related.length > 0 && (
        <div className="flex flex-col gap-4 mt-8">
          ...
          <div className="mt-4">
            <ToolGrid items={item.related} showSponsor={false} />
          </div>
        </div>
      )}
    </div>
