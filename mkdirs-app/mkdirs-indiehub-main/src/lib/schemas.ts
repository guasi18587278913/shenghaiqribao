 ...
 * submit tool
 */
export const baseSubmitToolSchema = {
  name: z
    .string()
    .min(1, { message: "Name is required" })
    .max(32, { message: "Name must be 32 or fewer characters long" }),
  ...
  categories: z
    .array(z.string())
    .min(1, { message: "Must select at least one category" }),
  subcategory: z.string().min(1, { message: "Must select a subcategory" }),
  imageId: z.string().min(1, { message: "Must upload an image" }),
};

export const SubmitToolSchema = SUPPORT_ITEM_ICON
  ? z.object({
      ...baseSubmitToolSchema,
      iconId: z.string().min(1, { message: "Must upload an icon" }),
    })
  : z.object(baseSubmitToolSchema);
