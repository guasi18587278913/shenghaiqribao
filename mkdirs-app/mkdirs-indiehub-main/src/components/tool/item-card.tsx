...
import { TAGS_NUMBER_IN_ITEM_CARD } from "@/lib/constants";
...
/**
 * ToolCard shows tool cover image
 */
export default function ToolCard({ item }: ToolCardProps) {
  ...
  const itemUrlPrefix = "/tool";
  const itemPageUrl = `${itemUrlPrefix}/${item.slug.current}`;
            ...
            <div className="relative w-full h-full">
              ...
              <div className="absolute left-2 bottom-2 opacity-100 transition-opacity duration-300">
                <div className="flex flex-col gap-2">
                  {/* categories, subcategory, only one category */}
                  {item.categories && item.categories.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {item.categories.map((category, index) => (
                        ...
                        >
                          {category.name} / {item.subcategory.name}
                        </span>
                      ))}
                    </div>
                  )}
      ...
      <div className="px-4 pb-4 flex justify-end items-center">
        {item.tags && item.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 items-center">
            {item.tags.slice(0, TAGS_NUMBER_IN_ITEM_CARD).map((tag, index) => (
              <Link
                key={tag._id}
            ...
            {item.tags.length > TAGS_NUMBER_IN_ITEM_CARD && (
