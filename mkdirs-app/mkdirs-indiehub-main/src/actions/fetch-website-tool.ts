...
import type { Category, Tag, ToolTag } from "@/sanity.types";
 ...
 */
export async function fetchWebsiteTool(url: string): Promise<ServerActionResponse> {
  try {
...
export const fetchWebsiteInfoWithAI = async (url: string) => {
  try {
    ...
    // );
    const tags = await sanityClient.fetch<ToolTag[]>(`*[_type == "toolTag"]`);
    ...
    const result = await generateObject({
      ...
      4. Select relevant tags from the available tags list (return array of names)
      5. Provide a screenshot image full URL, choose open graph image URL if possible
      6. Provide a website logo full URL, choose website favicon URL if possible
